...
public class GlobalExceptionAdvice {
     ...
     * @return 返回消息
     */
    @ExceptionHandler(DmBizException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ResponseEntity<RetResult<Object>> sendErrorResponse(DmBizException exception){
        log.error("业务请求异常: {}", getStackTrace(exception));
        return new ResponseEntity<>(new RetResult<>(exception.getDmErrorCode(),exception.getLocalizedMessage()),exception.getDmErrorCode().getHttpStatus());
    }
    /**
     * 单个参数校验（没有绑定对象）
